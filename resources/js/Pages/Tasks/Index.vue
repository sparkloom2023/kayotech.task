        ...
        <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div class="bg-white p-6 rounded-lg w-full max-w-md">
                <h2 class="text-xl font-bold mb-4">
                    {{ modalType === 'create' ? 'Create Task' : 'Edit Task' }}
                </h2>
    ...
    <div class="flex items-center">
        <div class="flex-1">
            ...
            <textarea
                ...
                class="w-full border rounded p-2 mb-1"
            ></textarea>
            <span v-if="form.errors[`subtasks.${index}.description`]" class="text-red-500 text-sm">
                {{ form.errors[`subtasks.${index}.description`] }}
            </span>
    ...
    setup() {
            ...
            form.subtasks = task.subtasks && Array.isArray(task.subtasks)
                ? task.subtasks.map(subtask => {
                    const mappedSubtask = {
                        id: subtask.id,
                        title: subtask.title || '',
                        description: subtask.description || '', // Ensure description is mapped
        ...
        function submitForm() {
            if (modalType.value === 'edit') {
                form.put(route('tasks.update', form.id, false) || `/tasks/${form.id}`, {
                    onSuccess: () => {
                        form.reset();
                ...
                form.post(route('tasks.store', undefined, false) || '/tasks', {
                    onSuccess: () => {
                        form.reset();
                        showModal.value = false;
                    },
        ...
        function deleteTask(task) {
            if (confirm('Are you sure you want to delete this task?')) {
                useForm({}).delete(route('tasks.destroy', task.id, false) || `/tasks/${task.id}`, {
                    onSuccess: () => {
        ...
        return { form, submitForm, deleteTask, editTask, openCreateModal, showModal, modalType, addSubtask, removeSubtask };
    },
};
</script>
